...
var FILEDIALOG_PARAM_PICKER_SCRIPT = "pickerscript";

var FILEDIALOG_CHCMD_ONCOMPLETE = "onPickerComplete",
...
var UI_SKYDRIVEPICKER = "skydrivepicker";

var ERROR_DESC_PICKER_TIMEOUT = "Loading SkyDrive picker is timed out.";
...
function appInitPlatformSpecific(properties) {          
    ...
    wl_app._redirect_uri = normalizeRedirectUrl(properties[AK_REDIRECT_URI], "WL.init");
...
SkyDrivePickerControl.prototype = {
    ...
    onRender: function (mouseDown, mouseIn) {
        var props = this._props,
            themeWhite = props[UI_PARAM_THEME] === UI_SIGNIN_THEME_WHITE,
            locale = wl_app._locale,
        ...
        button.style.backgroundColor = themeWhite ? whiteCode : blueCode;
        button.style.border = "solid 1px";
        button.style.borderColor = blueCode;
        ...
        text.style.fontFamily = "'Segoe UI', 'Segoe UI Web Regular', 'Helvetica Neue', 'BBAlpha Sans', 'S60 Sans', Arial, 'sans-serif'";
        text.style.fontSize = "12px";
        text.style.fontWeight = "bold";
        text.style.color = themeWhite ? blueCode : whiteCode;
        text.style.textAlign = "center";
        text.style.verticalAlign = "middle";
        text.style.marginLeft = rtl ? "0px" : "2px";
        text.style.marginRight = rtl ? "2px" : "0px";
    }
};
...
function buildSkyDrivePickerControlHtml(properties) {
    var mode = properties[FILEDIALOG_PARAM_MODE],
        ...
        buttonId = isOpenPicker ? "skydriveopenpickerbutton" : "skydrivesavepickerbutton",
        ...
        textHtml = "<span>" + buttonText + "</span>",
		buttonHtml = "<button id='" + buttonId + "' title='" + buttonTitle + "'>" + imgHtml + textHtml + "</button>";
    ...
    if (onClick) {
        var onMouseClick = function (e) {
            if (container.childNodes.length == 0) {
                detachDOMEvent(button, "click", onMouseClick);
                return;
            }
            onClick(e);
        };
        attachDOMEvent(button, "click", onMouseClick);
    }
...
AuthRequest.prototype = {
    ...
    _login: function () {
        ...
        request._promise = new Promise("WL.login", null, null);
...
(function () { 
    ...
    FilePickerOperation.prototype = {
        ...
        cancel: function(error) {
            ...
            if (!error) {
                error = createErrorResponse(
                    ERROR_REQ_CANCEL,
                    ERROR_DESC_CANCEL.replace("METHOD", this._props[API_INTERFACE_METHOD]));
            }
        ...
        _buildExternalScope: function () {
            ...
            scope += (op._props[FILEDIALOG_PARAM_SELECT] === "single") ? "single" : "multi";
            return scope;
        },
        ...
        _report: function() {
            ...
            selected_type = (foldersCount > 0 && filesCount > 0) ? "both" :
                (foldersCount > 0 ? "folder" :
                    (filesCount > 0 ? "file" : "none"));
...
UploadOperation.prototype._getStrategy = function (properties) {
    ...
    if (typeof(element) === TYPE_STRING) {
        ...
        // element will not be null. That check was done by validate parameters.
    }
    ...
    if (!(element instanceof HTMLInputElement) ||
        ...
        throw createInvalidParamValue(
                API_PARAM_ELEMENT, 
                interfaceMethod, 
                "It must be an HTMLInputElement with its type attribute set to " +
                "\"file\" (i.e., <input type=\"file\" />).");
    }
    ...
    if (element.files && window.FileReader) {
        ...
        self.setFileName(fileInput.name);
    ...
    var hasElementNameSet = element.name !== "";
    if (!hasElementNameSet) {
        element.name = FILE;
    }
    ...
    if (element.form === null) {
        ...
        errorMessage = 
            "It must be the only HTMLInputElement in its parent HTMLFormElement.";
    } else if (element.name !== FILE) {  // the input element must be named file
        errorMessage = 
            "Its name attribute must be set to \"file\" " +
            "(i.e., <input name=\"file\" />)."
    }
...
function XhrUploadStrategy(operation, uploadSource) {
    ...
    var self = this;
    self.upload = function (requestUrl) {
        var reader = null;
...
wl_app[API_X_HTTP_LIVE_LIBRARY] = "Web/DEVICE_" + trimVersionBuildNumber("5.5.8209.4000");
